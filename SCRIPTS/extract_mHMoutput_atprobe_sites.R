...
sites = read_sf(here("DATA","soilmoisture_probes.geojson"))

m = "FAO"
folders = list.files(here("BASIN_DATA/out/7339001/calibration_runs/",str_c(m,"_calibrated/")), full.names = TRUE)
folders
out = here("PROC","TABLE","calibrated_model_output","probes",m)
dir.create(out)
...
for (i in 1:length(folders)) {
  ...
  write_csv(df, here(out, str_c("mhm_output_probes_",model.it,".csv")))
}


# Model summary -----------------------------------------------------------
in.dir = here("PROC","TABLE","calibrated_model_output","probes",m)
in.dir
...
daily_summary
write_csv(daily_summary, here("PROC","TABLE", str_c("mhm_output_summary_probes_",m,".csv")))
