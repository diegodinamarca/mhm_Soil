...
# Extract data from soil moisture probes ----------------------------------
sites = read_sf(here("DATA","soilmosture_div.geojson")) %>% group_by(fid) %>% slice(1)

m = "FAO"
folders = list.files(here("BASIN_DATA/out/7339001/calibration_runs/",str_c(m,"_calibrated/")), full.names = TRUE)
folders
out = here("PROC","TABLE","calibrated_model_output","diviner",m)
dir.create(out)
...
for (i in 1:length(folders)) {
  ...
  df.et =  sites_extract_bylayer(flux, sites)
  names(df.et)[3:8] = paste0("aET_L", 1:6)
  
  # extract SWC
  flux = get_flux(folder, m, n = 3, daily = TRUE, process = FALSE)
  df.swc = sites_extract_bylayer(flux, sites)
  names(df.swc)[3:8] = paste0("swc_L", 1:6)
  
  df = full_join(df.et, df.swc)
  df$model = model.it
  
  write_csv(df, here(out, str_c("mhm_output_diviner_",model.it,".csv")))
}


# Model summary -----------------------------------------------------------
in.dir = here("PROC","TABLE","calibrated_model_output","diviner",m)
in.dir
...
daily_summary
write_csv(daily_summary, here("PROC","TABLE", str_c("mhm_output_summary_diviner_",m,".csv")))
